...
import { Phone, CheckCircle, Send, ShieldCheck, Clock, Loader2 } from "lucide-react";
...
export default function FPQuoteForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await submitSiteLead({
      source: SITE_LEAD_SOURCE.plumbingQuoteForm,
              ...
              {!submitted ? (
                <form onSubmit={handleSubmit} className="px-7 py-7 space-y-5">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 disabled:opacity-70 text-white py-4 rounded-xl font-extrabold text-sm shadow-lg shadow-sky-600/25 hover:shadow-sky-600/40 transition-all duration-200 hover:-translate-y-0.5"
                  >
                    {loading ? (
